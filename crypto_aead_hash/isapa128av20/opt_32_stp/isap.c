    ...
    const uint8_t *T, const uint8_t *T_star)
{
    ...
    // Calculate U
    to_bit_interleaving(s->l + 0, U64BIG(*(lane_t *)(T + 0)));
    to_bit_interleaving(s->l + 1, U64BIG(*(lane_t *)(T + 8)));
    s->x[2] = s0.x;
    s->x[3] = s1.x;
    s->x[4] = 0;
    P_PVP;
    ...
    // Calculate U'
    to_bit_interleaving(s->l + 0, U64BIG(*(lane_t *)(T_star + 0)));
    to_bit_interleaving(s->l + 1, U64BIG(*(lane_t *)(T_star + 8)));
    s->x[2] = s0.x;
    s->x[3] = s1.x;
    s->x[4] = 0;
    P_PVP;
    ...
    {
        eq_cnt += (s->b[0][i] == u0.b[i]);
    }
    for (size_t i = 0; i < 8; i++)
    {
        eq_cnt += (s->b[1][i] == u1.b[i]);
    }
    return eq_cnt;
}
